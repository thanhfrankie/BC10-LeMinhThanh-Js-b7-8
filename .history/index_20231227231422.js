...
function timSoNguyenToTrongMang(arr) {
    let soNguyenToTrongMang = [];

    for (let i = 0; i < arr.length; i++) {
        if (laSoNguyenTo(arr[i])) {
            soNguyenToTrongMang.push(arr[i]);
        }
    }

    return soNguyenToTrongMang;
}
function soNguyenToDauTien() {
    var soNguyenToTrongMang = [];
    ...
    ketqua.innerHTML = `Mảng sau khi đổi: ${soNguyenToTrongMang}`
}